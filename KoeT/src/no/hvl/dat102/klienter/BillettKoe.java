...
public class BillettKoe{	
	private final static int PROSESSTID = 120;  	// Gj.sn tid for � betjene en kunde
	...
	public static void main(String[]a){
		...
		for(int ant_luker=1; ant_luker <= MAKS_ANT_LUKER; ant_luker++){//Simuleringsl�kke
			
			// Nullstiller tid for alle luker, klokke over hver luke
			for(int i = 0; i<ant_luker; i++){
				lukeTid[i] = 0;
			}
			// Fyller k�en med alle kundene
			for(int i = 1; i<=ANT_KUNDER; i++){ // Gj.sn ankommer en kunde hvert 15.sek
				kundekoe.innKoe(new Kunde(i*15)); 
			}
			totalTid = 0;
			// Behandler alle kunder i k�en
			// Hver luke tar etter tur 1 kunde om gangen inntil k�en er tom
